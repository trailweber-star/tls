...
/** A non-specialty tile — e.g. a facility category like Pharmacy — that
    links straight to its own filtered search rather than /search?specialty=. */
export interface ExtraCategoryTile {
  id: string;
  name: string;
  description?: string;
  photo: string;
    ...
    <div className="grid grid-cols-2 gap-5 sm:grid-cols-4 sm:gap-6">
      ...
      {extra.map((tile) => (
        ...
        >
          ...
          <div className="flex flex-1 flex-col px-2.5 pb-2 pt-4">
            <h3 className="font-display text-[16px] font-bold leading-snug text-ink">{tile.name}</h3>
            {tile.description && (
              <p className="mt-1.5 text-[12.5px] leading-snug text-ink-muted">{tile.description}</p>
            )}
            <span className="mt-auto flex items-center gap-1.5 pt-4 text-[12.5px] font-bold text-teal-600">
              Explore
              <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" strokeWidth={2.5} />
